...
describe('libraryController', () => {
    ...
    it('getLibraryBookById redirects to the library list when the library book is missing', async () => {
        ...
        expect(req.flash).toHaveBeenCalledWith('error', 'Library book with ID 99 not found.');
    ...
    it('addBookToLibrary flashes success and redirects after adding the book to library', async () => {
        ...
        const res = { redirect: vi.fn() };
        const next = vi.fn();
        const newLibraryBook = { library_book_id: 5, book_id: 42, status: 'want_to_read' };
        mocks.addBookToLibrary.mockResolvedValue(newLibraryBook);

        await libraryController.addBookToLibrary(req, res, next);

        expect(req.flash).toHaveBeenCalledWith('success', 'Book added to library successfully.');
    ...
    it('showEditLibraryBookForm redirects when the library book is missing', async () => {
        ...
        expect(req.flash).toHaveBeenCalledWith('error', 'Library book with ID 7 not found.');
        expect(res.redirect).toHaveBeenCalledWith('/library');
    });
    ...
    it('updateLibraryBook flashes success and redirects to the updated library book', async () => {
        ...
        const res = { redirect: vi.fn() };
        const next = vi.fn();
        const updatedLibraryBook = { library_book_id: 1, book_id: 42, status: 'completed' };
        mocks.updateLibraryBook.mockResolvedValue(updatedLibraryBook);

        await libraryController.updateLibraryBook(req, res, next);

        expect(req.flash).toHaveBeenCalledWith('success', 'Library book updated successfully.');
    ...
    it('removeBookFromLibrary flashes success and redirects after removing the book', async () => {
        ...
        expect(req.flash).toHaveBeenCalledWith('success', 'Book removed from library successfully.');
        expect(res.redirect).toHaveBeenCalledWith('/library');
    });
