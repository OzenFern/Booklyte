...
describe('authorRepository', () => {
    ...
    it('findByOpenLibraryId looks up the author by Open Library ID', async () => {
        ...
        const author = { author_id: 3, openlibrary_id: 'OL1234A', name: 'Mary Shelley' };
        pool.query.mockResolvedValue({ rows: [author] });

        await expect(findByOpenLibraryId('OL1234A')).resolves.toEqual(author);
        expect(pool.query).toHaveBeenCalledWith('SELECT * FROM authors WHERE openlibrary_id = $1', ['OL1234A']);
    });
    ...
    it('getAuthorsByBookId returns every author connected to a book', async () => {
        ...
        const authors = [
            { author_id: 1, openlibrary_id: 'OL1', name: 'Author One', created_at: '2025-01-01' },
            { author_id: 2, openlibrary_id: 'OL2', name: 'Author Two', created_at: '2025-01-02' },
        ];
    ...
    it('getAuthorById fetches a single author by ID', async () => {
        ...
        const author = { author_id: 8, openlibrary_id: 'OL8', name: 'Isaac Asimov' };
        pool.query.mockResolvedValue({ rows: [author] });

        await expect(getAuthorById(8)).resolves.toEqual(author);
        expect(pool.query).toHaveBeenCalledWith('SELECT * FROM authors WHERE author_id = $1', [8]);
    });
    ...
    it('associateAuthorWithBook inserts the relationship row for a book-author join', async () => {
        ...
        expect(pool.query).toHaveBeenCalledWith('INSERT INTO book_authors (book_id, author_id) VALUES ($1, $2)', [20, 10]);
    });
